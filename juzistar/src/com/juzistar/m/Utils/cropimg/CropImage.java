...
package com.juzistar.m.Utils.cropimg;
        ...
        if (extras != null) {
            if (extras.getString(CIRCLE_CROP) != null) {
                if (Build.VERSION.SDK_INT > Build.VERSION_CODES.HONEYCOMB) {
                    mImageView.setLayerType(View.LAYER_TYPE_SOFTWARE, null);//todo api level fix
                }
                mCircleCrop = true;
                mAspectX = 1;
                mAspectY = 1;
...
//            mSaveUri = getImageUri(mImagePath);
            mSaveImagePath = getCropedImgPath(mImagePath);
            mSaveUri = getImageUri(mSaveImagePath);
...
//        }
    ...
    private void saveOutput(Bitmap croppedImage) {
        ...
        if (mSaveUri != null) {
            ...
            intent.putExtra(IMAGE_PATH, mSaveImagePath);
...
//            Toast.makeText(activity, noStorageText, 5000).show();
    ...
    private String getCropedImgPath(String originPath){
        String dir = Utils.getFileCacheDir();
        File file = new File(originPath);
        StringBuilder sb = new StringBuilder(dir);
        String toPath = sb.append("crop_").append(file.getName()).toString();
        return toPath;
    }
